...
createDate = async (req, res) => {
    try {
        ...
        const existingDates = await model.SamashrayanamDates.findAll({
            where: {
                [Op.or]: [
                    {
                        from_date: {
        ...
        await model.SamashrayanamDates.create(data);
    ...
    try {
        await model.SamashrayanamDates.findAll({
            ...
            if (result.length > 0) {
                ...
                return res.status(500).json({ message: "No price found for Samashrayanam. Please contact admin." });
            }
        }).catch((err) => {
...
getAllPrice = async (req, res) => {
    try {
        await model.SamashrayanamDates.findAll({order: [
            ['from_date', 'ASC']
        ]}).then((result) => {
            return res.status(200).json(result);
        }).catch((err) => {
...
deleteDate = async (req, res) => {
    try {
        ...
        await model.SamashrayanamDates.findOne({ where: { id: data["id"] } }).then((result) => {
            if (result) {
                model.SamashrayanamDates.destroy({ where: { id: data["id"] } }).then((x) => {
...
allBookings=async(req,res)=>{
    try {
        await model.SamashrayanamBookings.findAll({where:{is_approved:"Booked",is_paid:false}}).then((result) => {
            return res.status(200).json(result);
        }).catch((err) => {
...
newBooking = async (req, res) => {
    try {
        ...
        await model.SamashrayanamBookings.create(data).then((result) => {
...
updateStatus =async(req,res)=>{
    try {
       ...
       await model.SamashrayanamBookings.update(data,{where:{id:data["id"]}}).then((result) => {
...
myBookings = async (req, res) => {
    try {
        ...
        await model.SamashrayanamBookings.findAll({ where: { user_id: data["user_id"] } }).then((result) => {
            return res.status(200).json(result);
        }).catch((err) => {
            return res.status(500).json({ message: "Not able to get data.", error: err });
        });
...
getBookingDetail = async (req, res) => {
    try {
        ...
        await model.SamashrayanamBookings.findAll({ where: {id: data["id"] } }).then((result) => {
            return res.status(200).json(result);
        }).catch((err) => {
            return res.status(500).json({ message: "Not able to get data.", error: err });
        });
