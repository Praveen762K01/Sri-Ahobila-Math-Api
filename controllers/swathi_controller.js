...
getDatePrice = async (req, res) => {
    ...
    try {
        const prices = await model.SwathiPrice.findAll({
            where: {
                date: {
        ...
        if (prices.length > 0) {
            ...
            return res.status(500).json({ message: "No price found for Swathi. Please contact admin." });
        }
    } catch (error) {
        return res.status(500).json({ message: "Something Went Wrong, Please try again later.", error: error });
    }
...
getAllDatePrice = async (req, res) => {
    try {
        await model.SwathiPrice.findAll({order: [
            ['date', 'ASC']
        ]}).then((result) => {
...
allBookings = async (req, res) => {
    try {
        await model.SwathiBookings.findAll({where:{is_approved:"Booked",is_paid:false}}).then((result) => {
            return res.status(200).json(result);
        }).catch((err) => {
