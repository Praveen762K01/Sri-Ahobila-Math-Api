...
createDate = async (req, res) => {
    try {
        ...
        const existingDates = await model.HomeDolai_Master_Table.findAll({
            where: {
                [Op.or]: [
                    {
                        from_date: {
        ...
        await model.HomeDolai_Master_Table.create(data);
...
deleteDate = async (req, res) => {
    try {
        ...
        await model.HomeDolai_Master_Table.findOne({ where: { id: data["id"] } }).then((result) => {
            if (result) {
                model.HomeDolai_Master_Table.destroy({ where: { id: data["id"] } }).then((x) => {
...
myBookings = async (req, res) => {
    try {
        ...
        await model.HomeDolai_Transaction_Table.findAll({ where: { user_id: data["user_id"] } }).then((result) => {
            return res.status(200).json(result);
        }).catch((err) => {
            return res.status(500).json({ message: "Not able to get data.", error: err });
        });
