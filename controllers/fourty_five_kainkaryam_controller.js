...
createDatePrice = async (req, res) => {
    try {
        ...
        await model.FourtyFiveKainkaryamPrice.create(data).then((result) => {
...
getDatePrice = async (req, res) => {
    try {
        await model.FourtyFiveKainkaryamPrice.findAll().then((result) => {
...
deleteDatePrice = async (req, res) => {
    try {
        ...
        await model.FourtyFiveKainkaryamPrice.findOne({ where: { id: data["id"] } }).then((result) => {
            if (result) {
                model.FourtyFiveKainkaryamPrice.destroy({ where: { id: data["id"] } }).then((x) => {
...
allBookings = async (req, res) => {
    try {
        await model.FourtyFiveKainkaryamBookings.findAll().then((result) => {
...
myBookings = async (req, res) => {
    try {
        ...
        await model.FourtyFiveKainkaryamBookings.findAll({ where: { user_id: data["user_id"] } }).then((result) => {
            return res.status(200).json(result);
        }).catch((err) => {
            return res.status(500).json({ message: "Not able to get data.", error: err });
        });
...
getBookingDetail = async (req, res) => {
    try {
        ...
        await model.FourtyFiveKainkaryamBookings.findAll({ where: {id: data["id"] } }).then((result) => {
            return res.status(200).json(result);
        }).catch((err) => {
            return res.status(500).json({ message: "Not able to get data.", error: err });
        });
