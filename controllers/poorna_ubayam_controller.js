...
createPrice = async (req, res) => {
    try {
        ...
        await model.PoornaUbayamPrice.update({ is_active: false }, { where: {
            ...
            return res.status(200).json({ message: "Poorna Ubayam Price Created Successfully." });
        }).catch((err) => {
    ...
    try {
        await model.PoornaUbayamPrice.findOne({where:{
            ...
            if(result){
                ...
                return res.status(500).json({message:"No price found for Poorna Ubayam. Please contact admin."});
            }
        }).catch((err) => {
...
deletePrice = async (req, res) => {
    try {
        ...
        await model.PoornaUbayamPrice.findOne({ where: { id: data["id"] } }).then((result) => {
            if (result) {
                model.PoornaUbayamPrice.destroy({ where: { id: data["id"] } }).then((x) => {
...
bookings=async(req,res)=>{
    try {
       ...
       await model.PoornaUbayamBookings.findAll({where:{is_approved:"Booked",is_paid:false}}).then((result) => {
        return res.status(200).json(result);
       }).catch((err) => {
...
myBookings=async(req,res)=>{
    try {
       ...
       await model.PoornaUbayamBookings.findAll({where:{user_id:data["user_id"]}}).then((result) => {
        return res.status(200).json(result);
       }).catch((err) => {
        return res.status(500).json({ message: "Not able to get data.", error: err });
       });
